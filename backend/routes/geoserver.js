...
async function setupClassifiedCOGRasterTiles(rasterPath, layerName, classifications, workspace = config.geoserver.workspace, country) {
  try {
    ...
    // Set NoData value appropriately
    if (nanMatch) {
      noDataValue = 'nan'  // Keep as string to identify NaN case
      console.log(`🎭 Detected NaN NoData value: ${nanMatch[1]}`)
    } else if (noDataMatch) {
      noDataValue = parseFloat(noDataMatch[1])
    }
    
    console.log(`📈 Raster statistics: Min=${rasterMin}, Max=${rasterMax}, NoData=${noDataValue}`)
    console.log(`🔍 NoData detection details:`)
    console.log(`   - Numeric match found: ${!!noDataMatch}, Raw: "${noDataMatch ? noDataMatch[1] : 'none'}"`)
    console.log(`   - NaN match found: ${!!nanMatch}, Raw: "${nanMatch ? nanMatch[1] : 'none'}"`)
    console.log(`   - Final NoData value: ${noDataValue}`)
    console.log(`   - NoData type: ${typeof noDataValue}`)
    
    // Debug: Print relevant parts of gdalinfo output for NoData detection
    const noDataLines = gdalInfo.stdout.split('\n').filter(line => 
      line.toLowerCase().includes('nodata') || 
      line.toLowerCase().includes('nan') ||
      line.includes('NoData Value')
    )
    if (noDataLines.length > 0) {
      console.log(`🔍 Relevant gdalinfo lines for NoData:`)
      noDataLines.forEach(line => console.log(`   "${line.trim()}"`))
    }
    ...
    // Add no-data value with transparent color (enhanced handling for NaN)
    if (noDataValue !== null && noDataValue !== 'nan') {
      colorTableContent += `${noDataValue} 0 0 0 0\n`  // Transparent
      console.log(`🎭 Added transparent no-data entry: ${noDataValue} -> RGBA(0,0,0,0)`)
    } else if (noDataValue === 'nan') {
      console.log(`🎭 NaN NoData detected - will be converted to numeric NoData (-9999) for proper transparency`)
    } else {
    ...
    if (noDataValue === 'nan') {
      console.log(`🔧 Pre-processing NaN NoData values...`)
      const tempProcessedPath = path.join(path.dirname(rasterPath), `${layerName}_nan_processed.tif`)
      
      // Try multiple approaches to handle NaN values
      let preprocessSuccess = false
      
      // Approach 1: Try gdal_calc.py (most effective)
      try {
        const calcCommand = `gdal_calc.py -A "${rasterPath}" --outfile="${tempProcessedPath}" --calc="numpy.where(numpy.isnan(A),-9999,A)" --NoDataValue=-9999 --quiet`
        console.log(`🔄 Trying gdal_calc.py approach: ${calcCommand}`)
        await execAsync(calcCommand)
        preprocessSuccess = true
        console.log(`✅ gdal_calc.py successfully converted NaN values`)
      } catch (calcError) {
        console.warn(`⚠️ gdal_calc.py failed: ${calcError.message}`)
      }
      
      // Approach 2: Fallback to gdalwarp
      if (!preprocessSuccess) {
        try {
          const warpCommand = `gdalwarp -dstnodata -9999 "${rasterPath}" "${tempProcessedPath}"`
          console.log(`🔄 Trying gdalwarp approach: ${warpCommand}`)
          await execAsync(warpCommand)
          preprocessSuccess = true
          console.log(`✅ gdalwarp successfully processed raster`)
        } catch (warpError) {
          console.warn(`⚠️ gdalwarp failed: ${warpError.message}`)
        }
      }
      
      // Approach 3: Fallback to gdal_translate
      if (!preprocessSuccess) {
        try {
          const translateCommand = `gdal_translate -a_nodata -9999 "${rasterPath}" "${tempProcessedPath}"`
          console.log(`🔄 Trying gdal_translate approach: ${translateCommand}`)
          await execAsync(translateCommand)
          preprocessSuccess = true
          console.log(`✅ gdal_translate successfully processed raster`)
        } catch (translateError) {
          console.warn(`⚠️ gdal_translate failed: ${translateError.message}`)
        }
      }
      
      if (preprocessSuccess) {
        // Update paths and color table
        processedRasterPath = tempProcessedPath
        
        // Add numeric NoData entry to color table
        const numericNoDataEntry = '-9999 0 0 0 0\n'
        colorTableContent = numericNoDataEntry + colorTableContent
        await fs.writeFile(colorTablePath, colorTableContent)
        console.log(`🎭 Updated color table with numeric NoData entry: -9999 -> RGBA(0,0,0,0)`)
      } else {
        console.error(`❌ All NaN preprocessing approaches failed - proceeding with original raster`)
        console.warn(`⚠️ NaN values may still cause visualization issues`)
      }
    }
    
    // Apply color relief using gdaldem
    const gdalDemCommand = `gdaldem color-relief "${processedRasterPath}" "${colorTablePath}" "${tempColoredPath}" -alpha`
    console.log(`🎨 Executing color relief: ${gdalDemCommand}`)
    await execAsync(gdalDemCommand)
    
    // Second: Convert to COG format with alpha channel preservation
    ...
    // Step 9: Save legend file for future reference
    console.log(`📊 Step 9: Saving legend file...`)
    const legendData = {
    ...
    const processedDataDir = path.join(__dirname, '../data/processed')
    // The raster listing looks for ${layerName}_legend.json where layerName already includes '_classified'
    const legendPath = path.join(processedDataDir, `${layerName}_classified_legend.json`)
    
    try {
      // Ensure the processed directory exists
      await fs.mkdir(processedDataDir, { recursive: true })
      // Save legend file
      await fs.writeFile(legendPath, JSON.stringify(legendData, null, 2))
      console.log(`✅ Legend file saved: ${legendPath}`)
    } catch (legendError) {
      console.warn(`⚠️ Could not save legend file: ${legendError.message}`)
    }
    ...
    try {
      ...
      // Clean up NaN processed file if it exists
      if (noDataValue === 'nan') {
        const tempProcessedPath = path.join(path.dirname(rasterPath), `${layerName}_nan_processed.tif`)
        try {
          await fs.unlink(tempProcessedPath)
          console.log(`🧹 NaN processed file cleaned up: ${tempProcessedPath}`)
        } catch (nanCleanupError) {
          console.warn(`⚠️ Could not cleanup NaN processed file: ${nanCleanupError.message}`)
        }
      }
